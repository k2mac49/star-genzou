...
(function (root, factory) {
  ...
  var DEFAULTS = {
    ...
    levelG:       0,
    levelB:       0,
    /* チャンネル別のトーンカーブ。256要素の Float32Array (0-255 の値) か null。
  ...
  function apply(M, params, outInfo) {
    ...
    for (var i6 = 0; i6 < n; i6++) {
      ...
      /* 表引きは浮動小数のまま線形補間する。整数に丸めてから引くと、
         カーブで引き伸ばした範囲に値が1つも入らない明るさが生じ、
         ヒストグラムが櫛の歯状になる(=トーンジャンプ。実測でピン1本で
         空きビン 0 -> 13)。ここまで値は連続量で流れているので、丸めは最後の1回だけにする。 */
      /* それでも元が 8bit なので、強く引き伸ばした範囲には埋めようのない穴が残る
         (最大強度で空きビン 9)。存在しない階調は作れないので、引き伸ばし量に
         応じた振幅のディザで段差を細かい粒に置き換える。乱数は画素位置から
         決めるので、スライダーを動かしても模様がちらつかない。 */
      if (luR) {
        var ir = vr | 0, sr = ir >= 255 ? 0 : luR[ir + 1] - luR[ir];
        vr = ir >= 255 ? luR[255] : luR[ir] + sr * (vr - ir);
        if (sr > 1) vr += (((i6 * 2654435761) >>> 0) / 4294967296 - 0.5) * sr;
      }
      if (luG) {
        var ig = vg | 0, sg2 = ig >= 255 ? 0 : luG[ig + 1] - luG[ig];
        vg = ig >= 255 ? luG[255] : luG[ig] + sg2 * (vg - ig);
        if (sg2 > 1) vg += (((i6 * 2246822519) >>> 0) / 4294967296 - 0.5) * sg2;
      }
      if (luB) {
        var ib = vb | 0, sb = ib >= 255 ? 0 : luB[ib + 1] - luB[ib];
        vb = ib >= 255 ? luB[255] : luB[ib] + sb * (vb - ib);
        if (sb > 1) vb += (((i6 * 3266489917) >>> 0) / 4294967296 - 0.5) * sb;
      }
      out[q2] = vr; out[q2 + 1] = vg; out[q2 + 2] = vb;
      out[q2 + 3] = 255;
    }
