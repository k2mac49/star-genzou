...
- **RGB ヒストグラムを直接編集** — チャンネルごとにピンを置き、階調を圧縮・伸長できます（Q で影響範囲、ピンは個別に削除可）
- **エクスプローラ** — 読み込んだフォルダをサムネイルと露出情報つきで一覧表示
...
スライダーは単位を揃えてあります。「背景の明るさ」は測定値の「背景レベル」と同じ 0-255 で、
好みの調整（黒の残し方・彩度）は 0 を中心にした ± です。
